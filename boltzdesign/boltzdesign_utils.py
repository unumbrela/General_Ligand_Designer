...
def boltz_hallucination(
    ...
    # 检测是否为适配体设计模式
    if aptamer_config is not None:
        # 导入适配体设计模块
        from aptamer_design_utils import (
            initialize_aptamer_sequence, update_aptamer_sequence, 
            apply_aptamer_gradient_mask, extract_aptamer_sequence,
            record_aptamer_sequence_history, calculate_aptamer_constraints,
            create_aptamer_mask_and_chain_mask, get_aptamer_alphabet
        )
        print(f"🧬 适配体设计模式: {aptamer_config.aptamer_type} 适配体")
        
        # 调用适配体专用的hallucination函数
        return boltz_aptamer_hallucination(
            boltz_model, yaml_path, ccd_lib, aptamer_config, length, binder_chain,
            design_algorithm, recycling_steps, pre_iteration, soft_iteration, 
            soft_iteration_1, soft_iteration_2, temp_iteration, hard_iteration,
            semi_greedy_steps, learning_rate, learning_rate_pre, inter_chain_cutoff,
            intra_chain_cutoff, num_inter_contacts, num_intra_contacts,
            e_soft, e_soft_1, e_soft_2, alpha, pre_run, set_train, use_temp,
            disconnect_feats, disconnect_pairformer, mask_ligand, distogram_only,
            input_res_type, non_protein_target, increasing_contact_over_itr,
            loss_scales, optimize_contact_per_binder_pos, pocket_conditioning,
            chain_to_number, msa_max_seqs, optimizer_type, save_trajectory, noise_scaling
        )
...
def boltz_aptamer_hallucination(
    boltz_model, yaml_path, ccd_lib, aptamer_config, length, binder_chain,
    design_algorithm, recycling_steps, pre_iteration, soft_iteration, 
    soft_iteration_1, soft_iteration_2, temp_iteration, hard_iteration,
    semi_greedy_steps, learning_rate, learning_rate_pre, inter_chain_cutoff,
    ...
    # 初始化适配体序列 (角色互换: 适配体变成设计对象)
    data = initialize_aptamer_sequence(data, aptamer_config, length)
    
    name = yaml_path.stem
    target = parse_boltz_schema(name, data, ccd_lib)
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    boltz_model.train() if set_train else boltz_model.eval()
    print(f"设置为 {'train' if set_train else 'eval'} 模式")
    ...
    def get_batch(target, max_seqs=0, length=100, pocket_conditioning=False, keep_record=False):
        ...
        if pocket_conditioning:
            options = target.record.inference_options
            if options and hasattr(options, 'pocket_conditioning') and options.pocket_conditioning:
                features = featurizer.process(tokenized, training=False)  # 简化调用
            else:
                features = featurizer.process(tokenized, training=False)
        else:
            features = featurizer.process(tokenized, training=False)

        batch = {key: value if torch.is_tensor(value) else torch.from_numpy(value) for key, value in features.items()}
    ...
    # 获取batch数据
    batch_result = get_batch(target, max_seqs=msa_max_seqs, length=length, pocket_conditioning=pocket_conditioning, keep_record=True)
    if isinstance(batch_result, tuple):
        batch, structure = batch_result
    else:
        batch = batch_result
        structure = None
    
    # 排除非tensor对象，只处理tensor数据
    tensor_batch = {key: value for key, value in batch.items() if torch.is_tensor(value)}
    non_tensor_batch = {key: value for key, value in batch.items() if not torch.is_tensor(value)}
    batch = {key: value.unsqueeze(0).to(device) for key, value in tensor_batch.items()}
    batch.update(non_tensor_batch)  # 保留非tensor数据
    
    # 初始化适配体的res_type_logits
    if pre_run:
        batch['res_type_logits'] = batch['res_type'].clone().detach().to(device).float()
        # 为适配体链添加噪声初始化
        aptamer_mask = batch['entity_id'] == chain_to_number[aptamer_config.aptamer_chain]
        if aptamer_mask.any():
            noise_shape = batch['res_type'][aptamer_mask,:].shape
            # 只对允许的token添加噪声
            forbidden_mask = torch.zeros(batch['res_type'].shape[-1], device=device)
            forbidden_mask[aptamer_config.forbidden_tokens] = 1
            noise_logits = torch.distributions.Gumbel(0, 1).sample(noise_shape).to(device)
            noise_logits = noise_logits - forbidden_mask * 1e10
            batch['res_type_logits'][aptamer_mask,:] = noise_scaling * torch.softmax(noise_logits, dim=-1)
    else:
        batch['res_type_logits'] = batch['res_type'].clone().detach().to(device).float()

    # 设置MSA相关参数 (适配体设计通常使用单序列模式)
    batch['msa'] = batch['res_type_logits'].unsqueeze(0).to(device)
    batch['msa_paired'] = torch.ones(batch['res_type'].shape[0], 1, batch['res_type'].shape[1]).to(device)
    batch['deletion_value'] = torch.zeros(batch['res_type'].shape[0], 1, batch['res_type'].shape[1]).to(device)
    batch['has_deletion'] = torch.full((batch['res_type'].shape[0], 1, batch['res_type'].shape[1]), False).to(device)  
    ...
    # 使用适配体字母表
    alphabet = get_aptamer_alphabet(aptamer_config.aptamer_type)
    best_loss = float('inf')  
    min_loss = float('inf') 
    best_batch = None     
    ...
    def design(batch, iters=None, soft=0.0, e_soft=None, step=1.0, e_step=None,
               ...
               e_num_optimizing_binder_pos=1, learning_rate=1.0, inter_chain_cutoff=21.0,
               intra_chain_cutoff=14.0, mask=None, chain_mask=None, length=100,
               plots=None, loss_history=None, i_con_loss_history=None, con_loss_history=None,
               plddt_loss_history=None, distogram_history=None, sequence_history=None,
               pre_run=False, mask_ligand=False, distogram_only=False, predict_args=None,
               alpha=2.0, loss_scales=None, binder_chain='A', non_protein_target=False,
               increasing_contact_over_itr=False, optimize_contact_per_binder_pos=False,
               num_inter_contacts=2, num_intra_contacts=4, save_trajectory=False):

        # 优化调度参数
        m = {}
        if soft is not None and e_soft is not None:
            m["soft"] = (soft, e_soft)
        if step is not None and e_step is not None:
            m["step"] = (step, e_step)
        if temp is not None and e_temp is not None:
            m["temp"] = (temp, e_temp)
        if hard is not None and e_hard is not None:
            m["hard"] = (hard, e_hard)
        if num_optimizing_binder_pos is not None and e_num_optimizing_binder_pos is not None:
            m["num_optimizing_binder_pos"] = (num_optimizing_binder_pos, e_num_optimizing_binder_pos)

        opt = {k: s for k, (s, e) in m.items()}
        ...
        for i in range(iters):
            ...
            # 使用适配体序列更新函数 (核心修改)
            batch = update_aptamer_sequence(opt, batch, mask, aptamer_config, alpha, device)
            ...
            if 'plddt' in dict_out:
                plddt_loss = get_plddt_loss(dict_out['plddt'], mask_1d=chain_mask)
                total_loss = total_loss + 0.1 * plddt_loss
            ...
            if prev_sequence is not None and len(current_sequence) > 0 and len(prev_sequence) > 0:
                diff_count = sum(1 for a, b in zip(current_sequence, prev_sequence) if a != b)
                diff_percentage = (diff_count / max(len(current_sequence), len(prev_sequence))) * 100
            prev_sequence = current_sequence
            ...
            # 应用适配体梯度掩码 (核心修改)
            apply_aptamer_gradient_mask(batch, aptamer_config, chain_to_number, device)
            ...
            if batch['res_type_logits'].grad is not None:
                ...
                optimizer.zero_grad()
                current_lr = optimizer.param_groups[0]['lr']
                constraint_info = " ".join([f"{k}: {v.item():.3f}" for k, v in aptamer_constraints.items()])
                
                # 构建完整的损失信息
                loss_info_parts = []
                if 'i_con_loss' in locals():
                    loss_info_parts.append(f"i_con: {i_con_loss.item():.3f}")
                if 'con_loss' in locals():
                    loss_info_parts.append(f"con: {con_loss.item():.3f}")
                if 'plddt_loss' in locals():
                    loss_info_parts.append(f"plddt: {plddt_loss.item():.3f}")
                
                loss_info = ", ".join(loss_info_parts)
                if constraint_info:
                    loss_info = f"{loss_info}, {constraint_info}" if loss_info else constraint_info
                
                print(f"Epoch {i}: lr: {current_lr:.3f}, 序列: {current_sequence[:20]}{'...' if len(current_sequence) > 20 else ''}, 总损失: {total_loss.item():.2f}, {loss_info}")
        
        # 确保所有列表都有默认值
        if plots is None: plots = []
        if loss_history is None: loss_history = []
        if i_con_loss_history is None: i_con_loss_history = []
        if con_loss_history is None: con_loss_history = []
        if plddt_loss_history is None: plddt_loss_history = []
        if distogram_history is None: distogram_history = []
        if sequence_history is None: sequence_history = []
        
        return batch, plots, loss_history, i_con_loss_history, con_loss_history, plddt_loss_history, distogram_history, sequence_history, traj_coords_list, traj_plddt_list

    # 执行设计流程 (与原有逻辑相同但使用适配体函数)
    if pre_run:
        batch, plots, loss_history, i_con_loss_history, con_loss_history, plddt_loss_history, distogram_history, sequence_history, traj_coords_list, traj_plddt_list = design(
            batch, iters=pre_iteration, soft=1.0, e_soft=1.0, temp=1.0, e_temp=1.0, hard=0.0, e_hard=0.0, step=1.0, e_step=1.0, mask=mask, chain_mask=chain_mask, 
            learning_rate=learning_rate_pre, length=length, plots=plots, 
            loss_history=loss_history, i_con_loss_history=i_con_loss_history, 
            con_loss_history=con_loss_history, plddt_loss_history=plddt_loss_history, 
            distogram_history=distogram_history, sequence_history=sequence_history, 
            pre_run=pre_run, mask_ligand=mask_ligand, distogram_only=distogram_only, 
            predict_args=predict_args, loss_scales=loss_scales, binder_chain=aptamer_config.aptamer_chain,
            increasing_contact_over_itr=increasing_contact_over_itr, 
            optimize_contact_per_binder_pos=optimize_contact_per_binder_pos, 
            non_protein_target=True, inter_chain_cutoff=inter_chain_cutoff, 
        ...
        best_logits = batch['res_type_logits']
        best_seq = extract_aptamer_sequence(batch, aptamer_config, chain_to_number)
        print(f"预运行完成，最佳适配体序列: {best_seq}")
        print("开始主要设计阶段...")
    ...
    if design_algorithm == "3stages":
        ...
        batch, plots, loss_history, i_con_loss_history, con_loss_history, plddt_loss_history, distogram_history, sequence_history, traj_coords_list1, traj_plddt_list1 = design(
            batch, iters=soft_iteration, soft=1.0, e_soft=e_soft, temp=1.0, e_temp=1.0,
            hard=0.0, e_hard=0.0, step=1.0, e_step=1.0,
            num_optimizing_binder_pos=1, e_num_optimizing_binder_pos=8, mask=mask, 
            chain_mask=chain_mask, learning_rate=learning_rate, length=length, plots=plots, 
            loss_history=loss_history, i_con_loss_history=i_con_loss_history, 
            con_loss_history=con_loss_history, plddt_loss_history=plddt_loss_history, 
            distogram_history=distogram_history, sequence_history=sequence_history, 
            pre_run=pre_run, distogram_only=distogram_only, predict_args=predict_args, 
            loss_scales=loss_scales, binder_chain=aptamer_config.aptamer_chain, 
            increasing_contact_over_itr=increasing_contact_over_itr, 
            optimize_contact_per_binder_pos=optimize_contact_per_binder_pos, 
            non_protein_target=True, inter_chain_cutoff=inter_chain_cutoff, 
            intra_chain_cutoff=intra_chain_cutoff, num_inter_contacts=num_inter_contacts, 
        ...
        print("softmax(T=1) to softmax(T=0.01)")
        print('-'*100)
        print("设置 res_type_logits 到 logits")
        new_logits = (alpha * batch["res_type_logits"]).clone().detach().requires_grad_(True)
        batch['res_type_logits'] = new_logits
        optimizer = torch.optim.SGD([batch['res_type_logits']], lr=learning_rate)
        ...
        batch, plots, loss_history, i_con_loss_history, con_loss_history, plddt_loss_history, distogram_history, sequence_history, traj_coords_list2, traj_plddt_list2 = design(
            batch, iters=temp_iteration, soft=1.0, e_soft=1.0, temp=1.0, e_temp=0.01, 
            hard=0.0, e_hard=0.0, step=1.0, e_step=1.0,
            num_optimizing_binder_pos=8, e_num_optimizing_binder_pos=12, mask=mask, 
            chain_mask=chain_mask, learning_rate=learning_rate, length=length, 
            plots=plots, loss_history=loss_history, i_con_loss_history=i_con_loss_history, 
            con_loss_history=con_loss_history, plddt_loss_history=plddt_loss_history, 
            distogram_history=distogram_history, sequence_history=sequence_history, 
            pre_run=pre_run, distogram_only=distogram_only, predict_args=predict_args, 
            loss_scales=loss_scales, binder_chain=aptamer_config.aptamer_chain, 
        ...
        batch, plots, loss_history, i_con_loss_history, con_loss_history, plddt_loss_history, distogram_history, sequence_history, traj_coords_list3, traj_plddt_list3 = design(
            batch, iters=hard_iteration, soft=1.0, e_soft=1.0, hard=1.0, e_hard=1.0, 
            temp=0.01, e_temp=0.01, step=1.0, e_step=1.0,
            num_optimizing_binder_pos=12, e_num_optimizing_binder_pos=16, mask=mask, 
            chain_mask=chain_mask, learning_rate=learning_rate, length=length, 
            plots=plots, loss_history=loss_history, i_con_loss_history=i_con_loss_history, 
            con_loss_history=con_loss_history, plddt_loss_history=plddt_loss_history, 
            distogram_history=distogram_history, sequence_history=sequence_history, 
    ...
    for i, seq_entry in enumerate(data['sequences']):
        if aptamer_config.aptamer_type.lower() in seq_entry:
            if seq_entry[aptamer_config.aptamer_type.lower()]["id"][0] == aptamer_config.aptamer_chain:
                data['sequences'][i][aptamer_config.aptamer_type.lower()]['sequence'] = best_seq
                break
    
    print(f"🎯 最终设计的{aptamer_config.aptamer_type}适配体序列: {best_seq}")
    print(f"🔬 序列长度: {len(best_seq)}")
    
    # 提取目标蛋白质序列用于显示
    target_protein_seq = None
    for seq_entry in data['sequences']:
        if 'protein' in seq_entry:
            target_protein_seq = seq_entry['protein']['sequence']
            break
    
    if target_protein_seq:
        print(f"🧪 目标蛋白质序列: {target_protein_seq}")
        print(f"🔗 蛋白质长度: {len(target_protein_seq)} 氨基酸")
    
    # 验证设计质量
    from aptamer_design_utils import validate_aptamer_design
    validation_results = validate_aptamer_design(best_seq, aptamer_config.aptamer_type)
    print(f"📊 GC含量: {validation_results.get('gc_content', 0):.2%}")
    
    # 重新保存更新后的YAML文件
    with open(yaml_path, 'w') as f:
        yaml.dump(data, f, default_flow_style=False, sort_keys=False)
    print(f"📝 已更新YAML文件: {yaml_path}")
    ...
    try:
        ...
        final_output = _run_model(boltz_model, batch, predict_args_final)
        final_structure = batch.get('record', None)
        
        # 保存结构文件
        output_dir = os.path.dirname(yaml_path)
        structure_name = f"aptamer_{aptamer_config.aptamer_type.lower()}_final"
        
        save_confidence_scores(output_dir, final_output, final_structure, structure_name, 0)
        
        print(f"✅ 3D结构已保存:")
        print(f"📁 结构文件: {output_dir}/boltz_results_{structure_name}/predictions/{structure_name}/{structure_name}_model_0.cif")
        
        # 显示结构质量信息
        if 'plddt' in final_output:
            plddt_scores = final_output['plddt'][0].detach().cpu().numpy()
            aptamer_mask = batch['entity_id'] == chain_to_number[aptamer_config.aptamer_chain]
            aptamer_plddt = plddt_scores[aptamer_mask.cpu().numpy()]
            avg_plddt = float(aptamer_plddt.mean())
            print(f"📊 适配体平均置信度(pLDDT): {avg_plddt:.2f}")
            if avg_plddt > 70:
                print("✅ 结构质量: 高置信度")
            elif avg_plddt > 50:
                print("⚠️  结构质量: 中等置信度")
        ...
        print(f"⚠️  结构预测过程中出现错误: {e}")
        print("💡 序列设计已完成，但3D结构生成失败")
